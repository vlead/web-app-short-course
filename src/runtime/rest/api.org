...
def create_session_for_user(email, role, key):
  try:
    ...
    print("session_dict: %s" % session_dict)
...
def authorized():
  try:
      ...
      print("original session: %s" % session)

      role = session['role']
      print("role: %s" % role)
      print("calling create_session_for_user...")
      create_session_for_user(email, role, session['google_token'])
