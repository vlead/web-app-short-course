...
*** =Login= API
 ...
 #+NAME: login_route
 #+BEGIN_SRC python
@api.route('/login/<role>', methods=["GET"])
def login(role):
...
*** Google Callback response =API=
 ...
 #+BEGIN_SRC python
def create_session_for_user(email, role, key):
    try:
      session_dict = {'email': email,
                      'role': role,
                      'key': key }       

      print("\nsession_dict: %s" % session_dict)
...
def authorized():
    try:
        ...
        # BUG: roles[0].get("name") returns unicode encoded string, not native python
        # string. Why?
        role = str(roles[0].get("name")) if len(roles) == 1 else "user"

        
        create_session_for_user(email,
                                role,
                                session['google_token'])
        # TODO: merge both session objects in some way?
        # having both Flask's session and our SystemInterface
        # session is confusing 
        session["role"] = role
        
        print("create_session_for_user Succeeded.")
