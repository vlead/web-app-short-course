...
** Create Session
   - API to create login anssession
#+NAME: create_session
#+BEGIN_SRC python
...
def create_session():
	...
	key = session['google_token']
        local_session = {'email': email,
                         ...
                         'key': key }        
        try:            
            session_obj = SystemInterface.create_session_in_system(local_session)
            session['role'] = role
...
def create_users():
    if request.method == 'POST':
        ...
        data_json = json.dumps(request.get_json())
	data_dict = yaml.safe_load(data_json)
        local_user = {'email': data_dict['email'],
                      'role': data_dict['role'],
                      'name': data_dict['name'] }
	...
	key = session['google_token']
        local_session = {'email': email,
                         ...
                         'key': key }        
        try:            
            user_obj = SystemInterface.add_user(local_session, local_user)
            return jsonify({"status" : "success"})
        except Exception as e:
            abort(500, str(e))
