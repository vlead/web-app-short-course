...
# //FIXME: use this to create the different error handlers
def create_error_handler(app, status_code):

  @app.errorhandler(status_code)
  def error_handler(err):
    app.logger.error("error code = %s" % status_code)
    resp = None
    try:
        app.logger.error("error desc = %s" % err.description)
        resp = make_response(jsonify(error=err.description), status_code)
    except Exception:
        try:
            app.logger.error("error mesg = %s" % err.message)
            resp = make_response(jsonify(error=err.message), status_code)
        except Exception:
            resp = make_response(jsonify(error=str(err)), status_code)
...
def configure_errorhandlers(app):
    pass
    # FIXME: commented error handlers to get traceback
    # create_error_handler(app, 500)
    # create_error_handler(app, 500)
    # create_error_handler(app, 401)
    # create_error_handler(app, 400)
