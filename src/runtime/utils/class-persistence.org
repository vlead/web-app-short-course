...
'''
        ...
        cls.set_state = ClassPersistenceTemplate.mk_set_state(cls)
    ...
    def mk_set_state(cls):
        def fn(obj, level):
            ...
            for key in cls.sorted_state_vars:
                ...
                if value.__class__.__name__ is 'InstrumentedList' or \
                  value.__class__.__name__ is'list':
                    val_list = []
                    for val in value:
                        if hasattr(val.__class__, 'set_state') and \
                          level <= 2:
                            val.__class__.set_state(val, level+1)
                            
                        val_list.append(val)

                    obj.state[key] = val_list
               
                else:
                    if hasattr(value.__class__, 'set_state') and \
                      level <= 2:
                        value.__class__.set_state(value, level+1)
...
from runtime.objects.entities import Answer as Name
