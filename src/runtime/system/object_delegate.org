...
#+BEGIN_SRC python
    def get_user_by_email(self, email):
        user_list = filter(lambda user: user.get("email").get("email") == email,
                             self.user_set)
        if user_list:
            return user_list[0]
        else:
...
#+BEGIN_SRC python
    def test_get_user_by_email(self):
        ...
        user = self.obj_delegate.get_user_by_email(Config.admin_email)
        self.assertEqual(user.get("email").get("email"), 
                             Config.admin_email)

        user = self.obj_delegate.get_user_by_email("tt@kk.com")
