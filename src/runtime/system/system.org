...
** =GetUserByEmail=
   ...
   value.  Refer [[../../data-model/index.org::#get-user-by-email-auth][auth-check]] and referential [[../../data-model/index.org::#get-user-by-email-ref][inftegrity]] rules.
...
class DelRoleOfUser():
    ...
    def state_check(args):
        ...
        if not System.session_exists(session):
            ...
            raise StateError("number of roles of the user % is not greater "
                                 "than one" % role.to_client())

        elif (role.get("name") == Config.admin_role and
                  not user_with_admin_role_exists()):
            raise StateError("cannot delete admin role of the lone user in "
                                 "the system with admin role")
