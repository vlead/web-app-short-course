...
* Operations
  Operations provide the mechanism to change the state of the system.  While
  the system state is changed, it is imperative to maintain the invariant of
  the system.   Before the actual change in system is made, each operation goes
...
class AddUser():
    ...
    def state_check(args):
        user = args["user"]
        session = args["session"]
        if System.session_exists(session):
            ...
            raise StateError("session %s does not exist in System"
                             % session.user.get("name").get("name"))
...
class TestAddUser(TestCase):
    ...
    def test_add_user_raises_type_check(self):
        print "test_add_user_raises_type_check"
        admin_user=System.delegate.user_set[0]
        session = Session(user=admin_user)
        System.session_set.append(session)
...
class ShowUsers():
    ...
    @staticmethod
    def action(args):
        users = System.delegate.get_users()
        return users
 #+end_src
...
class TestShowUsers(TestCase):
    ...
    def test_show_users_by_admin(self):
        print "test_show_users__by_admin"
        admin_user=System.delegate.user_set[0]
        session = Session(user=admin_user)
        System.session_set.append(session)
        ...
        System.delegate.user_set.append(user)
        self.assertEquals(len(System.do("show_users", session=session)), 2)


    def test_show_users_by_user(self):
        print "test_show_users__by_user"
