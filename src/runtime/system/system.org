...
** =LoginFirstStep=
   A user is associated with multiple roles but each login session is
...
** =LoginSecondStep=
   ...
   sessions but each session of a user is uniquely identified by a role.  A
   user cannot have multiple sessions with a same role.
...
ops_table = {'add_user': AddUser,
             ...
             'set_email': SetEmail,
             'set_name': SetName,
             'login_first_step': LoginFirstStep,
