...
#+BEGIN_SRC python
    def get_role(self, role):
        return Role.apply_filters(name=role)[0]
...
#+BEGIN_SRC python
    def test_get_role(self):
         print "test_get_role"
         self.assertEqual(self.delegate.get_role("admin").get("name"),
                              "admin")
         self.assertEqual(self.delegate.get_role("user").get("name"),
                              "user")
...
#+BEGIN_SRC python
    def get_admin_user(self):
        return User.apply_filters(role=self.get_role("admin"))[0]
...
#+BEGIN_SRC python
    def get_user_by_email(self, email):
        user_list = User.apply_filters(email=email)
        if user_list:
            return user_list[0]
        else:
            None
