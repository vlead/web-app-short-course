...
class TestPersistenceDelegate(TestCase):
    ...
    def test_initialize_persistence_delegate(self):
         ...
         self.assertEqual(user_set[0].get("roles")[0].get("name"),
                              Config.admin_role)
...
#+BEGIN_SRC python
    def user_exists(self, user):
        if self.email_exists(user.get("email")):
            user_retrieved = self.get_user(email=user.get("email"))
            if user_retrieved is not None:
                return user.get("email") == user_retrieved.get("email")
        else:
            return False
...
#+BEGIN_SRC python
    def role_exists_in_system(self, role):
        return role == self.get_role(name=role.get("name"))
...
#+BEGIN_SRC python
    def test_user_with_admin_role_exists(self):
         ...
         self.delegate.del_user(self.delegate.get_user(email=Email(email=Config.admin_email)))
...
#+BEGIN_SRC python
    def add_user(self, user):
        ...
        for role in user.get("roles"):
            db_role = self.get_role(name=role.get("name"))
...
#+BEGIN_SRC python
    def test_add_user(self):
         ...
         user = User(name=name, email=email, 
                         roles=[self.delegate.get_role(name=Config.user_role)])
    ...
    def is_admin(self, user):
        ...
        if roles:
            ret_val = reduce(lambda a, b: a or b, \
                                map(lambda role: \
                                    role.get("name") == Config.admin_name, 
                                    roles))
        return ret_val
...
#+BEGIN_SRC python
    def test_is_admin(self):
         ...
         email.save()
         user = User(name=name, email=email,
                        roles=[self.delegate.get_role(name=Config.admin_role)])
...
#+begin_src python
    def get_admin_role(self):
        return self.get_role(name=Config.admin_role)
...
#+BEGIN_SRC python
    def test_get_admin_role(self):
         ...
         self.assertEqual(role.get("name"), Config.admin_role)
...
#+BEGIN_SRC python 
    def add_role_to_user(self, role, user):
        user = self.get_user(email=user.get("email"))
        roles = user.get("roles")
        role = self.get_role(name=role.get("name"))
        roles.append(role)
...
#+BEGIN_SRC python
    def test_add_role_to_user(self):
        user = self.delegate.get_user(email=Email(email=Config.admin_email))
