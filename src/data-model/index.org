...
|--------------------------------------+------------------------+----------------+--------------------|
| addUser(u:User, s:Session            | s.user.role=admin      | u:~user-set    | return user or     |
|                                      |                        | s:session-set  | Failure            |
|--------------------------------------+------------------------+----------------+--------------------|
| showUsers(s: Session)                | s.user.role=admin OR   | s:session-set  | return user-set or |
|                                      | s.user.role=user       |                | Failure            |
|--------------------------------------+------------------------+----------------+--------------------|
