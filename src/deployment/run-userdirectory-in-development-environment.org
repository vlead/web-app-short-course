...
* Creation of Virtual Environment
  ...
  + Create Virtual Environemnt in the =user-directory= directory
  #+BEGIN_EXAMPLE
  virtualenv venv
  #+END_EXAMPLE
...
* Set SQLALCHEMY_DATABASE_URI
  ...
  =build/code/src/config.py= so that the application can use the database.  In
  the example below, =root= is the user and =mysql= is the password.
  #+BEGIN_EXAMPLE
